...
**Example: 25 Custom Decorators for Angular**

```typescript
// ============================================================================
// 1. @Memoize - Cache method results
// ============================================================================
...
// ============================================================================
// 4. @Log - Automatic method logging
// ============================================================================
export function Log(level: 'info' | 'warn' | 'error' | 'debug' = 'info') {
  return function (
    target: any,
    propertyKey: string,
    descriptor: PropertyDescriptor
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          lastError = error;
          console.warn(`Retry attempt ${attempt}/${maxAttempts}`, error);
...
export function Measure(threshold?: number) {
  return function (
    ...
    descriptor.value = function (...args: any[]) {
      ...
      const className = target.constructor.name;
      const message = `${className}.${propertyKey}: ${duration.toFixed(2)}ms`;
...
export class DataComponent {
  @Measure(100) // Warn if takes more than 100ms
...
// ============================================================================
// 7. @Timeout - Enforce execution time limit
// ============================================================================
...
export class SlowService {
  @Timeout(5000)
  async fetchWithTimeout(): Promise<any> {
    return this.http.get('/api/slow-endpoint').toPromise();
  }
}

// ============================================================================
// 8. @Deprecated - Mark methods as deprecated
// ============================================================================
  ...
  return function (
    ...
    descriptor.value = function (...args: any[]) {
      const warning = message || 
        `${className}.${propertyKey} is deprecated${version ? ` since v${version}` : ''}`;
      console.warn('⚠️ DEPRECATED:', warning);
...
export class UserService {
  @Deprecated('Use getUserById instead', '2.0.0')
...
// ============================================================================
// 9. @Required - Validate required inputs
// ============================================================================
...
// ============================================================================
// 10. @ConfirmAction - Require user confirmation
// ============================================================================
...
export class AdminComponent {
  @ConfirmAction('Delete all user data? This cannot be undone!')
  deleteAllUsers(): void {
...
// ============================================================================
// 11. @Authorized - Check user permissions
// ============================================================================
export function Authorized(requiredRoles: string[]) {
  return function (
    ...
    descriptor.value = function (...args: any[]) {
      const authService = this.authService || this.injector?.get(AuthService);
...
export class InitService {
  @Once()
  initialize(): void {
    console.log('Initializing app (only runs once)');
    // Initialization logic
  }
}

// ============================================================================
// 13. @Bind - Automatically bind method context
// ============================================================================
...
// ============================================================================
// 14. @Validate - Parameter validation
// ============================================================================
export function Validate(validator: (value: any) => boolean, errorMsg: string) {
  return function (
    target: any,
    propertyKey: string,
    ...
    target[propertyKey] = function (...args: any[]) {
      if (!validator(args[parameterIndex])) {
        throw new Error(`${errorMsg} at parameter ${parameterIndex}`);
      }
      return originalMethod.apply(this, args);
    };
  };
...
class UserService {
  createUser(
    @Validate((val) => val && val.length > 0, 'Name cannot be empty') name: string,
    @Validate((val) => val && val.includes('@'), 'Invalid email') email: string
  ): void {
...
// ============================================================================
// 15. @Cache - Advanced caching with expiration
// ============================================================================
...
// ============================================================================
// 16. @AsyncQueue - Queue async operations
// ============================================================================
export function AsyncQueue() {
  return function (
    ...
    descriptor.value = function (...args: any[]) {
      queue = queue.then(() => originalMethod.apply(this, args));
      return queue;
    };
...
// ============================================================================
// 17. @ReadOnly - Make property read-only
// ============================================================================
...
// ============================================================================
// 18. @MinMax - Validate numeric range
// ============================================================================
...
// ============================================================================
// 19. @Freeze - Prevent object modification
// ============================================================================
...
export class StateService {
  @Freeze()
  getState(): AppState {
    return { user: { name: 'John' }, count: 0 };
  }
}

// ============================================================================
// 20. @Delay - Add artificial delay
// ============================================================================
...
// ============================================================================
// 21. @TrackChanges - Track property changes
// ============================================================================
export function TrackChanges(callback?: (oldValue: any, newValue: any) => void) {
  return function (target: any, propertyKey: string) {
    let value: any;
    ...
    Object.defineProperty(target, propertyKey, {
      ...
      set: (newValue: any) => {
        ...
        console.log(`${propertyKey} changed:`, { oldValue, newValue });
      },
      enumerable: true,
      configurable: true
    });
...
export class FormComponent {
  @TrackChanges((old, newVal) => console.log('Username changed!'))
  username = '';
}

// ============================================================================
// 22. @Singleton - Ensure single instance
// ============================================================================
...
// ============================================================================
// 23. @Enumerable - Control property enumeration
// ============================================================================
...
// ============================================================================
// 24. @AutoUnsubscribe - Automatic subscription cleanup
// ============================================================================
...
@Component({ selector: 'app-data' })
export class DataComponent {
  subscription1 = this.service.getData().subscribe();
  subscription2 = this.service.getMore().subscribe();
  // Both will be automatically unsubscribed
}

// ============================================================================
// 25. @RateLimit - Limit call frequency
// ============================================================================
export function RateLimit(maxCalls: number, timeWindow: number) {
  return function (
    target: any,
    propertyKey: string,
    descriptor: PropertyDescriptor
    ...
    descriptor.value = function (...args: any[]) {
      ...
      if (calls.length >= maxCalls) {
        ...
        const retryAfter = timeWindow - (now - oldestCall);
