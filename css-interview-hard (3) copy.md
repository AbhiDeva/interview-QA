...
/* 1. Can't use in media queries */
/* BAD */
@media (min-width: var(--breakpoint)) { }

/* 2. Can't concatenate strings */
/* BAD */
background: url('images/' var(--img-name) '.jpg');

/* 3. Property-specific (can't use everywhere) */
/* BAD */
--width: width;
element {
  var(--width): 100px; /* Doesn't work */
}
...
:root {
  ...
  --component-button-bg: var(--app-primary-color);
}
...
### Q15: Explain CSS Grid subgrid, container queries, and other cutting-edge features.

**Expected Answer:**
```css
/* SUBGRID (Limited browser support) */

/* Problem: Nested grids don't align with parent grid */
.parent-grid {
  display: grid;
  grid-template-columns: 1fr 2fr 1fr;
  gap: 20px;
...
/* Use case: Card layouts with aligned content */
.card-list {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 20px;
...
.container {
  ...
  container-type: normal; /* No container queries */
}
...
@container (min-width: 400px) {
  .element {
    ...
    height: 50cqmax; /* 50% of larger dimension */
  }
}
...
/* Style parent when it has specific child */
.card:has(img) {
  display: grid;
  grid-template-columns: 1fr 2fr;
}
...
.article:has(> h2):has(> p) {
  /* Article with both h2 and p as direct children */
}

/* Quantity queries */
ul:has(> li:nth-child(10)) {
  /* Style UL when it has 10+ items */
  columns: 2;
}
...
/* Specificity = highest selector in list */
:is(h1, h2, h3, h4, h5, h6) {
  line-height: 1.2;
}
...
:where(h1, h2, h3, h4, h5, h6) {
  line-height: 1.2; /* Specificity: (0,0,0,0) */
}

/* Useful for easy overriding */
:where(.button) {
  background: blue; /* Easy to override */
}

.special-button {
  background: red; /* Overrides above */
}
...
/* Layers respect order, not specificity */
@layer base {
  #id.class.class { color: red; } /* High specificity */
}

@layer utilities {
  .text-blue { color: blue; } /* Low specificity but wins! */
}
...
/* Direction-agnostic (works with RTL) */
...
.element {
  ...
  border-inline-start: 2px solid blue; /* Works in any direction */
}
...
.scroll-container {
  scroll-snap-type: x mandatory; /* x-axis, must snap */
  overflow-x: scroll;
  display: flex;
}
...
scroll-snap-type: x mandatory; /* Must snap */
scroll-snap-type: y proximity; /* Snap if close enough */
...
.element {
  ...
  color: lch(50% 100 0); /* Lightness, chroma, hue */
...
/* Upcoming feature (some browsers) */
.card {
  padding: 1rem;
...
1. **Build Real Projects** - Theory is important, but hands-on experience is crucial
2. **Study Browser DevTools** - Know how to debug CSS issues
3. **Practice CodePen/JSFiddle** - Quick prototyping skills
4. **Read Specifications** - MDN, W3C specs for deep understanding
5. **Follow CSS News** - Stay updated with new features
...
4. **Consider Edge Cases** - Different browsers, screen sizes
5. **Discuss Trade-offs** - Pros and cons of approaches
6. **Show Problem-Solving** - Debug systematically
7. **Mention Accessibility** - Show you care about all users
8. **Be Honest** - If unsure, explain how you'd research

**Common Interview Formats:**

- **Live Coding** - Build a component from scratch
- **Debug Existing Code** - Find and fix CSS issues
- **System Design** - Architect CSS for large applications
- **Code Review** - Critique provided CSS
- **Theoretical Questions** - Explain concepts in depth

---

**Good luck with your CSS interviews! 🎨🚀**